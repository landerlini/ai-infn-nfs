...
echo "Python FastAPI server started in the background"

while true; do

  # Check if NFS is STILL running by recording it's PID (if it's not running $pid_nfs will be null):
  pid_nfs=`pidof rpc.mountd`
  ...
  # Check if FastAPI is STILL running by recording it's PID (if it's not running $pid_http will be null):
  pid_http=`pidof python3`
  # If it is not, lets kill our PID1 process (this script) by breaking out of this while loop:
  # This ensures Docker observes the failure and handles it as necessary
  if [ -z "$pid_http" ]; then
    echo "NFS has failed, exiting, so Docker can restart the container..."
